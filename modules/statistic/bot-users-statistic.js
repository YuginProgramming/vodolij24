...
const botUsersStatistic = async () => {
  ...
  const topUsersMessage = topUsers
    .map((user) => {
      return `ID: ${user.id}, Імя: ${user.name}, ДН: ${
        user.birthdayDate
      }, ТЕЛ: ${user.phone}, Карта: ${
        user.cards
  ...
  const usersWaterTotal = usersWithTotals.reduce(
    (sum, user) => sum + user.userTotal,
    0
  );

  const summaryString = `Користувачів боту: ${usersQuantity},\nКількість налитої води, користувачами боту, за добу: ${usersWaterTotal.toFixed(
    0
  )} літрів.`;
...
const botWeeklyUsersStatistic = async () => {
  ...
  bot.sendMessage(
    dataBot.topId,
    `Топ 10 користувачів за тиждень:\n${topUsersMessage}`
  );
...
export { botUsersStatistic, botWeeklyUsersStatistic, botMonthlyUsersStatistic };
