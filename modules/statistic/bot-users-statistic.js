...
} from "../../models/bot-transactions.js";
...
const getPersonalRankMessage = async (cardId) => {
  ...
  if (!card || card.litersPerDay === 0) {
    ...
    rankMessage = `🏆 *Рейтинг:* Ви *#${
      cardIndex + 1
    }* у рейтингу користувачів цього місяця!`;
  } else {
    const betterThanPercent = (
      ((cards.length - cardIndex) / cards.length) *
      100
    ).toFixed(1);
